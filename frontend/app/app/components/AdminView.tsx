...
export default function AdminView() {
  ...
  const handleBulkResetTrial = () => {
    ...
    confirm(`Reset trial for ${selectedIds.size} user?`, () =>
  ...
  const exportAllCsv = async () => {
    try {
      ...
      if (!token) return;
      const params = buildFilters();
      const response = await fetch(`${API_URL}/api/admin/users/export-all?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!response.ok) throw new Error('Export all gagal.');
      const blob = await response.blob();
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = 'admin-users-all.csv';
      link.click();
    } catch {
      setToast('Gagal export semua users.');
    } finally {
      setLoading(false);
    }
  };

  const exportSearchCsv = async () => {
    if (!searchQuery.trim()) {
    ...
    try {
      ...
      params.set('page', String(page));
      params.set('per_page', String(perPage));
      const response = await fetch(`${API_URL}/api/admin/users/export-search?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!response.ok) throw new Error('Export search gagal.');
      const blob = await response.blob();
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = 'admin-users-search.csv';
      link.click();
    } catch {
      setToast('Gagal export hasil pencarian.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <table className="w-full text-sm">
            <thead className="text-xs text-gray-400 uppercase tracking-wide">
              <tr className="text-left">
                ...
                <th className="py-2 pr-4">Trial remaining</th>
            ...
            <tbody>
              {loading && (
                <tr>
                  <td colSpan={7} className="py-4 text-gray-500">Memuat data...</td>
                </tr>
              )}
              {!loading && users.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-4 text-gray-500">Tidak ada data.</td>
                </tr>
              )}
