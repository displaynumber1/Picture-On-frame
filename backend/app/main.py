...
from fastapi import FastAPI, APIRouter, File, UploadFile, HTTPException, Depends, Header, Response, Form, Request, WebSocket, WebSocketDisconnect, BackgroundTasks  # type: ignore
...
# Load environment variables early (before importing services that read env).
load_env()
...
from app.services.face_detection import has_human_face
from app.services.motion_logic import (
...
APP_NAME = os.getenv("APP_NAME", "picture-on-frame-ai-studio")
...
async def get_current_user_raw(request: Request, authorization: Optional[str] = Header(None)) -> Dict[str, Any]:
...
async def get_current_user_optional(request: Request, authorization: Optional[str] = Header(None)) -> Optional[Dict[str, Any]]:
    """Return current user if Authorization is present; otherwise None."""
...
def _is_admin_user(current_user: Dict[str, Any]) -> bool:
    """Check admin role from Supabase profile."""
    ...
    if not user_id:
        return False
    profile = get_user_profile(user_id)
    role_from_db = (profile or {}).get("role") or (profile or {}).get("role_user") or "user"
    return role_from_db == "admin"


async def require_admin(current_user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    # SECURITY: enforce admin role from DB profile (not token metadata).
    if current_user.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    return current_user


async def require_admin_or_reminder_token(
    request: Request,
    current_user: Optional[Dict[str, Any]] = Depends(get_current_user_optional),
) -> None:
    """
    Security: allow reminder job either via admin auth or a dedicated reminder token.
    This keeps the endpoint protected while still supporting cron calls.
    """
    if current_user:
...
# Initialize Gemini API Client lazily in app.services.gemini_service
# This keeps app startup light and avoids failures when GEMINI_API_KEY is missing.

# Database path
BACKEND_ROOT = Path(__file__).resolve().parents[1]
DB_PATH = BACKEND_ROOT / 'premium_studio.db'
AUTPOST_TEMP_DIR = BACKEND_ROOT / 'temp_videos'
AUTPOST_TEMP_DIR.mkdir(parents=True, exist_ok=True)
# SECURITY: restrict file access to a safe base directory.
SAFE_VIDEO_BASE = Path(os.getenv("AUTPOST_VIDEO_BASE", str(AUTPOST_TEMP_DIR))).resolve()
SAFE_TEMP_BASE = Path(os.getenv("AUTPOST_TEMP_BASE", str(AUTPOST_TEMP_DIR))).resolve()
...
AUTPOST_TRENDS_CSV = BACKEND_ROOT / "trends.csv"
...
LEGACY_IMAGE_COST = int(os.getenv("LEGACY_IMAGE_COST", "75"))
LEGACY_VIDEO_COST = int(os.getenv("LEGACY_VIDEO_COST", "5"))
...
def _get_profile_for_user(current_user: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    """Security: always resolve profile server-side before sensitive operations."""
    user_id = current_user.get("id")
    if not user_id:
...
def _require_coins(profile: Dict[str, Any], cost: int) -> int:
    """Security: enforce coins availability server-side."""
    coins = int(profile.get("coins_balance", 0) or 0)
    if coins < cost:
        raise HTTPException(
            status_code=403,
            detail=f"Insufficient coins. You need at least {cost} coins to continue."
        )
    return coins
...
def _log_request(
    ...
    logging.getLogger("app.request").info("request", extra=payload)
...
RATE_LIMIT_BUCKETS: Dict[str, _TokenBucket] = {}
RATE_LIMIT_LOCK = threading.Lock()


def _rate_limit_identifier(request: Request, user_id: Optional[str]) -> str:
    if user_id:
...
def enforce_rate_limit(request: Request, user_id: Optional[str], endpoint_key: str) -> None:
    # NOTE: In-memory limiter; use Redis for multi-instance deployments.
    limit = RATE_LIMIT_AUTH_LIMIT if user_id else RATE_LIMIT_ANON_LIMIT
    refill_rate = limit / RATE_LIMIT_WINDOW_SECONDS
    key = f"{endpoint_key}:{_rate_limit_identifier(request, user_id)}"
    with RATE_LIMIT_LOCK:
        bucket = RATE_LIMIT_BUCKETS.get(key)
        if not bucket:
            bucket = _TokenBucket(limit, refill_rate)
...
def _score_video_with_llm(
    ...
    try:
        ...
        if AUTPOST_LLM_PROVIDER == "gemini":
            raw = (generate_text_content(AUTPOST_LLM_MODEL, prompt) or "").strip()
            if not raw:
                return None
...
async def request_context_middleware(request: Request, call_next):
    ...
    try:
        ...
        _log_request(request, request_id, 500, duration_ms)
        raise
    duration_ms = int((time.perf_counter() - start_time) * 1000)
...
async def readiness_check():
    ...
    supabase_ok = bool(os.getenv("SUPABASE_URL")) and bool(os.getenv("SUPABASE_ANON_KEY"))
    checks["supabase_env"] = {"ok": supabase_ok}
    if not supabase_ok:
        ready = False

    ffmpeg_required = os.getenv("FFMPEG_REQUIRED", "false").lower() in {"1", "true", "yes", "on"}
    ffmpeg_ok = False
    try:
        ffmpeg_ok = check_ffmpeg_available()
    except Exception:
        ffmpeg_ok = False
    checks["ffmpeg"] = {"ok": ffmpeg_ok, "optional": not ffmpeg_required}
...
async def ensure_access(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    return {
        ...
        "user_id": user_id,
        "email": email,
        "role_user": (profile or {}).get("role_user") or (profile or {}).get("role") or "user",
        "trial_upload_remaining": (profile or {}).get("trial_upload_remaining"),
        "coins_balance": (profile or {}).get("coins_balance")
    }
...
A high-resolution video of the watch on the table, with subtle light glints reflecting off the glass as the camera breathes slightly.
        ...
        return generate_video_prompt_from_image(image_base64, image_mime, prompt_text)
    except Exception as e:
        error_msg = str(e)
...
TECHNICAL: Shot on Phase One IQ4, 150MP, sharp focus, 8K, high-frequency texture detail, zero digital noise.
    ...
    try:
        ...
        try:
            response = generate_imagen_content(
                enhanced_prompt,
                temperature=0.7,
                top_p=0.95,
                top_k=40
...
async def generate_images_legacy(
    request: GenerateRequest,
    current_user: Dict[str, Any] = Depends(get_current_user)
):
    """Generate 4 variations of studio images"""
    # Security: enforce auth + coins server-side for legacy generation.
    user_id, profile = _get_profile_for_user(current_user)
    _require_coins(profile, LEGACY_IMAGE_COST)
    results = []
    had_error = False
    for i in range(4):
        try:
...
A high-resolution video showing the product in this setting, with subtle natural movement and realistic lighting effects."""
    ...
    # Deduct coins only after the request completes successfully.
    if not had_error:
        update_user_coins(user_id, -LEGACY_IMAGE_COST)
    return results
    ...
    try:
        ...
        if "multipart/form-data" in content_type:
            ...
            try:
                ...
                # Read uploaded file content with streaming-safe size check
                file_name = image_file.filename
                total_size = 0
                chunks: List[bytes] = []
                while True:
                    chunk = await image_file.read(UPLOAD_CHUNK_SIZE)
                    if not chunk:
                        break
...
async def generate_video_saas(
    ...
    try:
        ...
        enforce_rate_limit(http_request, user_id, "generate-video-saas")
...
async def billing_reminders_run(
    request: Request,
    _: None = Depends(require_admin_or_reminder_token)
):
    # Security: require admin auth or a valid reminder token.
    threshold_days = int(os.getenv("SUBSCRIPTION_RENEW_WINDOW_DAYS", "5"))
...
def _count_active_autopost_tasks(user_id: str) -> int:
    if not user_id:
        return 0
    conn = get_db_connection()
    try:
        row = conn.execute(
            ...
            FROM autopost_videos
            WHERE user_id = ?
              AND status IN ('QUEUED', 'IN_PROGRESS', 'WAITING_RECHECK', 'PROCESSING', 'RETRYING')
            """,
            (user_id,)
        ).fetchone()
...
autopost_service = AutopostService(
    AutopostDeps(
        ...
        active_tasks=_count_active_autopost_tasks
    )
)
app.include_router(create_autopost_router(autopost_service, get_current_user, enforce_rate_limit))
...
async def autopost_download_video(
    ...
    real_path = Path(row["file_path"]).resolve()
    if not real_path.exists():
        raise HTTPException(status_code=404, detail="Video file not found")
    if not real_path.is_file() or not real_path.is_relative_to(SAFE_VIDEO_BASE):
        raise HTTPException(status_code=403, detail="Invalid file path")
    logger.info(f"[AUTPOST] user={user_id} downloaded video_id={video_id}")
    return FileResponse(
        real_path,
...
async def autopost_complete_task(
    ...
    allowed_previous = ("IN_PROGRESS", "PROCESSING", "RETRYING")
    if row["status"] in ("POSTED", "FAILED"):
        conn.close()
        raise HTTPException(status_code=409, detail="Task already completed")
    if row["status"] not in allowed_previous:
        conn.close()
        raise HTTPException(status_code=400, detail=f"Invalid state transition from {row['status']}")
    ...
    try:
        if row["file_path"]:
            real_path = Path(row["file_path"]).resolve()
            if not real_path.is_relative_to(SAFE_TEMP_BASE):
                raise HTTPException(status_code=500, detail="Unsafe file path detected")
            if real_path.exists():
                real_path.unlink()
    except HTTPException:
        raise
    ...
    logger.info(f"[AUTPOST] user={user_id} completed video_id={video_id} status={status}")
    await _broadcast_autopost_event(user_id, "autopost.updated", {"id": video_id, "status": status})
...
async def autopost_retry_task(
    ...
    allowed_retry_states = ("FAILED", "CANCELLED")
    if row["status"] not in allowed_retry_states:
        conn.close()
        raise HTTPException(
            status_code=400,
            detail=f"Task can only be retried if status in {allowed_retry_states}"
        )

    _update_autopost_record(conn, video_id, status="QUEUED", error=None)
    conn.commit()
    conn.close()
    logger.info(f"[AUTPOST] user={user_id} retry task video_id={video_id}")
...
async def autopost_retry_variant(
    ...
    if row["status"] == "QUEUED":
        conn.close()
        raise HTTPException(status_code=409, detail="Task already queued")
    allowed_retry_variant = ("FAILED",)
    if row["status"] not in allowed_retry_variant:
        conn.close()
        raise HTTPException(status_code=400, detail="Retry variant allowed only when FAILED")
    ...
    conn.commit()
    conn.close()
    logger.info(f"[AUTPOST] user={user_id} retry-variant video_id={video_id}")
    await _broadcast_autopost_event(user_id, "autopost.updated", {"id": video_id, "status": "QUEUED"})
...
async def admin_status(current_user: Dict[str, Any] = Depends(require_admin)):
    # Security: /api/admin/* must require admin role.
    user_id = current_user.get("id")
    role = get_user_role_from_profile(user_id) or "admin"
    return {"is_admin": True, "role": role}


@debug_router.get("/profile-status")
...
async def admin_subscription_update(
    ...
    profile_before = get_profile_by_user_id(target_user_id)
    ...
    if trial_remaining is not None:
        before = int(profile_before.get("trial_upload_remaining") or 0)
        after = int(profile.get("trial_upload_remaining") or 0)
        delta = after - before
        if delta != 0:
            action = "ADD_TRIAL" if delta > 0 else "SUB_TRIAL"
            try:
                insert_admin_adjustment(
                    admin_user_id=current_user.get("id") or "unknown",
                ...
                logger.error("Failed to insert admin adjustment", exc_info=True)
    conn = get_db_connection()
    _log_admin_audit(
        conn,
...
async def admin_reset_trial(
    ...
    profile_before = get_profile_by_user_id(user_id)
    ...
    conn.commit()
    conn.close()
    if profile_before and updated:
        before = int(profile_before.get("trial_upload_remaining") or 0)
        after = int(updated.get("trial_upload_remaining") or 0)
        delta = after - before
        if delta != 0:
            action = "ADD_TRIAL" if delta > 0 else "SUB_TRIAL"
            try:
...
async def admin_bulk_reset_trial(
    ...
    updated_count = 0
    for uid in unique_ids:
        profile_before = get_profile_by_user_id(uid)
        update_user_trial_remaining(uid, 3)
        profile_after = get_profile_by_user_id(uid)
        if profile_before and profile_after:
            before = int(profile_before.get("trial_upload_remaining") or 0)
            after = int(profile_after.get("trial_upload_remaining") or 0)
            delta = after - before
            if delta != 0:
                action = "ADD_TRIAL" if delta > 0 else "SUB_TRIAL"
                try:
...
@app.post("/api/admin/users/{user_id}/add-coins")
async def admin_add_coins(
    ...
    if delta_int <= 0 or delta_int > 100000:
        raise HTTPException(status_code=400, detail="delta must be between 1 and 100000")
    if not reason:
        raise HTTPException(status_code=400, detail="reason is required")

    updated = update_user_coins(user_id, delta_int)
    try:
        insert_admin_adjustment(
            admin_user_id=current_user.get("id") or "unknown",
            target_user_id=user_id,
    ...
    return {
        "id": user_id,
        "coins_balance": int((updated or {}).get("coins_balance") or 0)
    }


@app.post("/api/admin/users/{user_id}/sub-coins")
async def admin_sub_coins(
    user_id: str,
    request: Request,
    payload: Dict[str, Any],
    current_user: Dict[str, Any] = Depends(require_admin)
...
@app.post("/api/admin/users/bulk-add-coins")
async def admin_bulk_add_coins(
    request: Request,
    payload: Dict[str, Any],
    current_user: Dict[str, Any] = Depends(require_admin)
):
    ...
    updated_count = 0
    for uid in unique_ids:
        updated = update_user_coins(uid, delta_int)
        if updated:
            updated_count += 1
        try:
            insert_admin_adjustment(
...
@app.post("/api/admin/users/bulk-sub-coins")
async def admin_bulk_sub_coins(
    request: Request,
    payload: Dict[str, Any],
    current_user: Dict[str, Any] = Depends(require_admin)
):
...
async def generate_video(
    ...
    try:
        # Security: enforce auth + coins server-side for legacy generation.
        user_id, profile = _get_profile_for_user(current_user)
        _require_coins(profile, LEGACY_VIDEO_COST)
        ...
        update_user_coins(user_id, -LEGACY_VIDEO_COST)
